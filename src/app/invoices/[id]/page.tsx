...
export default async function InvoicePage({ params }: { params: { id: string } }) {
  ...
  return <main className="mx-auto max-w-4xl p-4 sm:p-8 print:p-0">
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-6 sm:p-10 print:border-0 print:p-0">
      <header className="flex justify-between gap-6 border-b border-slate-200 pb-6"><div className="flex items-start gap-4"><Image src="/logo.jpg" alt="TSM Mobiles" width={88} height={88} className="h-20 w-20 rounded-lg object-cover print:h-16 print:w-16" priority /><div><h1 className="text-2xl font-bold text-slate-900">{company.name}</h1><p className="mt-1 whitespace-pre-line text-sm text-slate-600">{company.address}</p><p className="text-sm text-slate-600">{company.phone} {company.email ? `· ${company.email}` : ""}</p>{company.taxId && <p className="text-sm text-slate-600">Tax ID: {company.taxId}</p>}</div></div><div className="text-right"><h2 className="text-xl font-bold tracking-wide text-slate-900">INVOICE</h2><p className="mt-2 font-semibold">{invoice.invoiceNumber}</p><p className="text-sm text-slate-600">Date: {formatDate(invoice.date)}</p>{invoice.dueDate && <p className="text-sm text-slate-600">Due: {formatDate(invoice.dueDate)}</p>}</div></header>
